...
<div class="aw-container-wrap">
				...
				<div class="col-sm-12 col-md-9 aw-main-content">
					...
					<div class="aw-mod aw-topic-bar" id="question_topic_editor" data-type="question" data-id="3">
						<div class="tag-bar clearfix">
                            <h1>
                                <a id="fontsize" class="pull-left">标题</a>
                                <span class="font24"><?php echo ($content["posts_title"]); ?></span>
                            </h1>
						</div>
					</div>
					<!-- end 圈子bar -->
					<div class="aw-mod aw-question-detail aw-item">
						...
						<div class="mod-head">
                            ...
                            <span class="font16"><?php echo ($content["posts_intr"]); ?></span>

						</div>
                        <div class="inners clearfix">
                            <div class="row">
                                <div class="shibox shiboximg col-xs-4 col-sm-4 col-md-4">
                                    <a href="/Public/<?php echo ($content["posts_pic"]); ?>" target="_blank" data-fancybox-group="thumb" rel="lightbox">
                                        <img class="img-polaroidst img-responsive" src="/Public/<?php echo ($content["posts_pic"]); ?>" alt="" original="" style="display: block;">
                                    </a>
                                </div>
                            </div>
                        </div>
						...
						<div class="mod-footer">
							<div class="meta">
								...
								<a href="/index.php/home/list/chargepost/posts_id/<?php echo ($content["posts_id"]); ?>/users_id/<?php echo ($users_id); ?>">
									<span class="text-color-999">投诉</span>
								</a>
								</div>
													<!--评论-->
								<div style="margin-top: 20px;">
									<div><p style="font-size: 20px;font-weight: 200">评论</p></div>
									<?php if(is_array($comment)): $i = 0; $__LIST__ = $comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div  style="margin-bottom: 30px;float: left;">
										<div class="aw-user-imgs" style="float: left;margin-left: 30px;">
 ...
 $usersid=$vo[users_id]; $tmp['users_id']=$usersid; $toName=M('users')->where($tmp)->field('users_pic')->find(); echo $toName['users_pic']; ?>" alt=""><p>
												<?php
 $usersid=$vo[users_id]; $tmp['users_id']=$usersid; $toName=M('users')->where($tmp)->field('users_name')->find(); echo $toName['users_name']; ?>
											</p></a>
                                            <div class="userline hidden-xs" id="userline_0">

                                            </div>
										</div>
										<div style="float: left;margin-left: 50px;width: 520px;">
											<p><?php echo ($vo["comments_content"]); ?></p>
											<p style="text-align: right"><?php echo ($vo["comments_time"]); ?></p>
										</div>
					...
					<div class="list-page" style="float: left;margin: 0 auto; width: 692.5px"><?php echo ($pages); ?></div>
										<!-- 评论编辑器 -->
					<div class="aw-mod aw-replay-box question">
						<a name="answer_form"></a>
						<th class="aw-padding10">
							<form id="comment_index" method="post" action="/Home/list/getcomment">
								<input type="hidden" name="id" value="<?php echo ($posts_id); ?>">
							<textarea name="comment" class="common-textarea" id="comment" cols="20" style="width: 98%;" rows="5"></textarea></th>
						<div style="margin-left: 630px;"><input class="btn btn-primary btn6 mr10" value="评论" onclick="location.href='/index.php/Home/list/getcomment'" type="submit"></form>
							</div>
											</div>
					<!-- end 评论编辑器 -->
						...
						<div class="mod-body">
							<dl>
								<dt class="pull-left aw-border-radius-5">
									<a href="/index.php/home/personal/index2/id2/<?php echo ($content[users_id]); ?>"><img alt="埃菲尔铁塔" src="/Public<?php
 $usersid=$content[users_id]; $tmp['users_id']=$usersid; $toName=M('users')->where($tmp)->field('users_pic')->find(); echo $toName['users_pic']; ?>"></a>
								</dt>
								<dd class="pull-left">
									<a class="aw-user-name" href="/index.php/home/personal/index2/id2/<?php echo ($content[users_id]); ?>"><?php
 $usersid=$content[users_id]; $tmp['users_id']=$usersid; $usersName=M('users')->where($tmp)->field('users_name')->find(); echo $usersName['users_name']; ?></a>
							...
							<ul style="font-size:12px;">
								<li> 发帖数量 : <em class="aw-text-color-orange">
                                    <?php
 ...
 $usersid=$content[users_id]; $tmp['users_id']=$usersid; $usersComment=M('users')->where($tmp)->field('users_comment')->find(); echo $usersComment['users_comment']; ?></em></li>
					...
					<div class="aw-mod question-status">
						...
						<div class="mod-body">
							<ul>
								<li>最新活动: <span class="aw-text-color-blue"><?php echo ($content["posts_time"]); ?></span></li>
								<li>浏览: <span class="aw-text-color-blue"><?php echo ($content["view_num"]); ?></span></li>
