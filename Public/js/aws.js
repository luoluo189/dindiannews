var AWS =
{
					...
					case 'reply_question':
						...
						if (result.rsm.ajax_html)
						{
							...
							if ($('.aw-replay-box.question').length)
							{
								if (USER_ANSWERED)
								{
									$('.aw-replay-box').append('<p align="center">一个话题只能评论一次, 你可以在发言后 ' + ANSWER_EDIT_TIME + ' 分钟内编辑评论过的内容</p>');
								}
							}
						}
	...
	dialog: function (type, data, callback)
	{
		switch (type)
		{
			...
			case 'commentEditshang':
				var template = Hogan.compile(AW_TEMPLATE.editCommentBoxshang).render(
				{
					'answer_id': data.answer_id,
					'attach_access_key': data.attach_access_key
				});
			...
			case 'foxupday':
				var template = Hogan.compile(AW_TEMPLATE.foxupday).render(
				{
					'foxid': data
				});
			break;
			...
			case 'shangta':
				if(!G_USER_ID){
					AWS.dialog('ajaxLogin');
					return false;
				}
				var template = Hogan.compile(AW_TEMPLATE.shangtaBox).render(
				{
					'url':G_BASE_URL + '/dashang/ajax/shangta/',
					'rmb': data.rmb,
					'user_name': data.user_name,
					'item_id':data.item_id,
					'item_trueid':data.item_trueid,
					'item_type':data.item_type
				});
				break;
			case 'shangtakan':
				...
				var template = Hogan.compile(AW_TEMPLATE.shangtakanBox).render(
				{
					'url':G_BASE_URL + '/dashang/ajax/shangta/',
					'rmb': data.rmb,
					'kanrmb': data.kanrmb,
		...
		if (template)
		{
			...
			switch (type)
			{
				...
				case 'foxreply':
					var editor = CKEDITOR.replace( 'foxreply_message' );
				break;
				...
				break;
				case 'shangta':
					$(document).on('click', '.qianbox_tem .btn', function()
					{
						var _this = $(this), url = G_BASE_URL + '/dashang/ajax/shangta/';
						$.post(url,
						{
							'item_id' :  _this.attr('data-item_id'),
							'item_trueid' : _this.attr('data-item_trueid'),
							'item_type' : 'answer',
							'shangnum' :  _this.attr('data-shangnum')
						}, function (result)
						{
							if (result.errno == 1)
							{
							...
							{
								$('.error_message').html(_t(result.err));
					...
					$('#showdy').click(function()
					{
						$("#formbox").fadeIn();
					});
					$('#shangnum').keyup(function(){
						var re = /^-?\d+\.?\d{0,2}$/;
						if ($('#shangnum').val() != "") { 
							if (!re.test($('#shangnum').val())) { 
								$('#shangnum').val('');
							} 
						} 
					});
